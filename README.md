...
PictoMan is a real-time multiplayer drawing game that combines creativity, strategy, and AI. Players create an account, log in, and are automatically matched with a random opponent for a head-to-head challenge.

At the start of each round, an AI generates a drawing prompt that both players must illustrate within a five-minute time limit. When the timer expires, a second AI evaluates the submitted drawings against the prompt and declares the winner based on relevance and quality.
